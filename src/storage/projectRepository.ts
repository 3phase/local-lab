...
const db = () => openDB('locallab', 2, {
  upgrade(d) {
    if (!d.objectStoreNames.contains('projects')) d.createObjectStore('projects', {keyPath: 'id'});
    if (!d.objectStoreNames.contains('datasets')) d.createObjectStore('datasets', {keyPath: 'projectId'})
  }
});
...
export async function saveProject(p: Project) {
  ...
  await tx.objectStore('projects').put({
    ...p,
    dataset: {...p.dataset, rows: [], rawRows: undefined},
...
export async function getProject(id: string) {
  ...
  if (dataset) {
    knownDatasets.add(id);
    return {...project, dataset} as Project
  }
  return project;
}
